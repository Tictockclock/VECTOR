...
def start_picoscenes():
    ...
    # Start PicoScenes as a subprocess
    cmd = f"""PicoScenes \"-d debug; -i {config["picoscenes"]["monID1"]} --mode logger  --output {config["picoscenes"]["NIC_save_file1"]}; -i {config["picoscenes"]["monID2"]} --mode logger  --output {config["picoscenes"]["NIC_save_file2"]} -q\""""
...
def master_handler():
    ...
    time.sleep(1)
    send_file(config["picoscenes"]["NIC_save_file2"])
